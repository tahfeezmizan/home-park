...
import { saveProperties } from '../utlity/localStorage';
...
const PropertieDetails = () => {
    ...
    return (
        <div className='w-full lg:w-9/12 xl:w-8/12 mx-auto px-3 py-10 md:px-0'>
            ...
            <div className="">
                <h2 data-aos="fade-right" data-aos-duration="1700" className="text-2xl md:text-4xl font-bold mb-3">{card.estate_title}
                    <span className='bg-yellow-500 text-white text-sm rounded-full capitalize px-2 py-1 ml-2'>{card.status}</span>
                </h2>
                <p data-aos="fade-right" data-aos-duration="1800" className=" flex items-center gap-2 text-gray-400">Categories: <span className="text-black font-semibold">{card.segment_name}</span></p>

                <div className="flex justify-between items-end -mt-2 md:-mt-9 pb-10">
                    <p data-aos="fade-right" data-aos-duration="1900" className=" flex items-center gap-2 text-gray-600"><span className="text-blue-600"><CiLocationOn /></span>{card.location}</p>
                ...
                <div className="col-span-4">
                    ...
                    <div className="border p-6 rounded-md">
                        <div className="flex justify-between items-end border-b pb-2 mb-6">
                            <p data-aos="fade-right" data-aos-duration="2100" className="text-gray-600 text-base">Property ID: <span className='font-bold text-black'>OP-S{card.id}</span></p>
                            <button onClick={handleAddWishlist} className='bg-blue-200 p-2 rounded-full hover:bg-blue-500 hover:text-white duration-200'>
                                <FaRegHeart />
                            </button>
                        </div>

                        <div data-aos="fade-right" data-aos-duration="2200" className="flex gap-7 flex-col md:flex-row md:items-end border-b pb-8">
                            <p className="text-blue-500 text-base flex items-center gap-1"><BsHouseCheck />< span className='font-medium text-gray-500'>{card.facilities[0]}</span></p>
                            <p className="text-blue-500 text-base flex items-center gap-1"><BsHouseCheck />< span className='font-medium text-gray-500'>{card.facilities[1]}</span></p>
                            <p className="text-blue-500 text-base flex items-center gap-1"><BsHouseCheck />< span className='font-medium text-gray-500'>{card.facilities[2]}</span></p>
                            <p className="text-blue-500 text-base flex items-center gap-1"><BsArrowsMove />< span className='font-medium text-gray-500'>{card.area} Sq ft</span></p>
                        </div>
